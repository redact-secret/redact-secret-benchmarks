...
import { generateCase, hash } from './model.mjs';
import { describeCase, describeVariant, summaries } from './reporting.mjs';

export async function runEvaluation({ cases, methods, operators, scanners, provenance = {}, onProgress = () => {} }) {
    ...
    for (const scanner of scanners) {
      ...
      try {
        ...
        observations.push({ id: scanner.id, version, mode: scanner.mode,
          status: error?.message === 'unavailable' ? 'unavailable' : 'error',
...
export function exitCode(report, { strict = false, failOnAssertions = false } = {}) {
