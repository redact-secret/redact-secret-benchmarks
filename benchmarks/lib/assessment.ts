...
export function classifyFixture(category: string, f: Fixture): Assessment {
  ...
  if (category === 'detector-coverage') {
    ...
    if (['bearer-token', 'connection-string', 'otpauth-uri', 'generic-token', ...CONTEXT_GATED, 'azure-devops-personal-access-token', 'new-relic-license-key'].includes(family) && !(family === 'datadog-application-key' && matches(family, value)))
