...
export const contracts: Record<string, FormatContract> = {
  ...
  // neither's evidence is used for the other, per docs/specs/decisions/2026-09-20-
  ...
  'discord-bot-token': { tier: 'T2', pattern: '^[A-Za-z0-9_-]{24}\\.[A-Za-z0-9_-]{6}\\.[A-Za-z0-9_-]{27}$', validate: value => /^[0-9]+$/.test(Buffer.from(value.split('.', 1)[0], 'base64url').toString('latin1')), corroboration: [th('discordbottoken/discordbottoken')], review: 'Discord\'s own single documented example matches this three-segment base64url shape exactly, corroborated by TruffleHog\'s discordbottoken detector. Gitleaks\'s discord-api-token and discord-client-secret rules target different, unrelated Discord credential shapes and are not cited.', unprobeable: unprobeable('docs.discord.com/developers/reference shows one example token in an Authorization header and states no segment count, encoding, length or alphabet. An example is not a grammar.') },
  ...
  // docs/specs/decisions/2026-09-20-add-firebase-server-key-detection-and-client-
  ...
  'generic-token': { tier: 'T3', references: [], review: 'An arbitrary literal in a sensitive field is a masking-policy case, not a provider-format ground truth.', twinSource: provider('https://github.com/redact-secret/redact-secret-benchmarks/blob/main/docs/specs/decisions/2026-09-20-extend-twins-to-assignment-context.md', 'context-twin decision (#36)', 'no provider exists for an arbitrary literal. The recorded decision is that the twin keeps the value and mutates exactly one property of the assignment context; silence is project policy, never a format claim', '2026-09-20') },
};
...
/** `context` keeps the value and mutates one property of the assignment context (docs/specs/decisions/2026-09-20-extend-twins-to-assignment-context.md). */
 ...
 * docs/specs/decisions/2026-09-21-check-lexical-separability.md.
 */
...
/** Decision 2 default (docs/specs/decisions/2026-09-21-add-untargeted-benign-corpus.md): every `real-world-shapes`
...
const REAL_WORLD_SHAPE = 'Real-world-shaped synthetic content, authored locally and not ingested from any external source (docs/specs/decisions/2026-09-21-add-untargeted-benign-corpus.md). No provider or tool evidence grounds its silence: expected silence is project policy (T3).';
 ...
 * docs/specs/decisions/2026-09-21-add-untargeted-benign-corpus.md.
 */
