...
export const contracts: Record<string, FormatContract> = {
  ...
  'datadog-application-key': { tier: 'T2', corroboration: [th('datadogtoken/datadogtoken'), gl], review: 'Datadog documents the DD-APPLICATION-KEY header and DD_APPLICATION_KEY environment variable but no character-class grammar. The bare 40-character shape, gated on a same-line datadog/dd marker, is corroborated by both TruffleHog\'s datadogtoken detector and gitleaks\'s datadog-access-token rule.', unprobeable: unprobeable('Re-checked 2026-09-22 (redact-secret#645): Datadog documents ddapp_ as the prefix of new-format application keys, a shape this legacy 40-hex contract does not cover, and states nothing mutable for the legacy value; its v1 OpenAPI 40/40 constraint sits on a field named hash, not read as the key value.', '2026-09-22') },
...
export function classifyFixture(category: string, f: Fixture): Assessment {
  ...
  if (category === 'detector-coverage') {
    ...
    if (['bearer-token', 'connection-string', 'otpauth-uri', 'generic-token', ...CONTEXT_GATED, 'azure-devops-personal-access-token', 'new-relic-license-key'].includes(family)) return policy(contracts[family ?? ''].review!, family);
