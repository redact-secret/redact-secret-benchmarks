...
export const contracts: Record<string, FormatContract> = {
  ...
  // neither's evidence is used for the other, per docs/decisions/2026-09-20-
  // scope-supabase-management-token-and-secret-key-independence.md.
  ...
  'vercel-token': { tier: 'T0', corroboration: [th('vercel/vercel')], candidateSource: provider('https://vercel.com/docs/accounts/access-tokens', 'vcp_/vci_/vca_/vcr_/vck_ prefixed opaque tokens', 'prefixes are now documented per credential type (personal/integration/app-access/app-refresh/API-key, via the token-format changelog); the body is stated to be an opaque format not intended to be human-readable, with no length or alphabet given', '2026-09-20'), review: 'Re-checked 2026-09-20 (#45): Vercel now documents all five prefixes (vcp_/vci_/vca_/vcr_/vck_) as visual/scanning markers, correcting the prior "no prefixes documented" reading, but explicitly states the body is opaque by design — there is no length or alphabet to document. The pinned TruffleHog detector still matches only a contextual, unprefixed 24-character token, not these five shapes. Tracked in redact-secret#516 (Vercel credential taxonomy audit), which directs against authoring a detector ahead of its conclusion.', unprobeable: unprobeable('The positive is T0 pending. Vercel documents the prefixes as scanning markers but states the body is intentionally opaque, so no lexical grammar exists to mutate; deferred to redact-secret#516.') },
  ...
  // google-api-key); no dedicated contract for those here, per
  // docs/decisions/2026-09-20-add-firebase-server-key-detection-and-client-
  // config-discrimination.md.
...
export function classifyFixture(category: string, f: Fixture): Assessment {
  ...
  if (category === 'detector-coverage') {
    ...
    if (family === 'linear-token' && value.startsWith('lin_oauth_'))
      return pending('Re-checked 2026-09-20 (#45): Linear documents no OAuth access-token grammar, and neither pinned tool registers a lin_oauth_ pattern — trufflehog\'s linearapi and gitleaks\'s linear-api-key detectors both match only lin_api_. Pending until a pinned scanner registration or Linear documentation describes this shape.', family);
    if (family === 'stripe-token' && /^(?:sk_org_|whsec_)/.test(value))
      return pending('Re-checked 2026-09-20 (#45): Stripe\'s key-types page names sk_org_ organization keys and the webhooks page documents whsec_ as the signing-secret prefix, but neither page nor the pinned trufflehog (`[rs]k_live_[a-zA-Z0-9]{20,247}`, live-only) or gitleaks (`(?:sk|rk)_(?:test|live|prod)_[a-zA-Z0-9]{10,99}`, no org alternative) stripe rules establish a body length or alphabet for either prefix. Tracked in redact-secret#513. Pending until tool corroboration or provider grammar exists for this prefix.', family);
    if (family === 'slack-token' && value.startsWith('xwfp-'))
      return pending('Re-checked 2026-09-20 (#45): the Slack tokens page documents the xwfp- prefix for workflow tokens but states no section widths or alphabet for it, the same gap already recorded for xoxb-; the pinned trufflehog slack detector covers only xoxb-/xoxp-/xoxa-/xoxr-, and gitleaks\'s slack-webhook-url rule matches only the full hooks.slack.com/workflows/... URL, not a bare token. Tracked in redact-secret#512. Pending until tool corroboration or provider body evidence exists.', family);
