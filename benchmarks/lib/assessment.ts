...
export function classifyFixture(category: string, f: Fixture): Assessment {
  ...
  if (category === 'detector-coverage') {
    ...
    if (['bearer-token', 'connection-string', 'otpauth-uri', 'generic-token', ...CONTEXT_GATED, 'azure-devops-personal-access-token'].includes(family)) return policy(contracts[family ?? ''].review!, family);
