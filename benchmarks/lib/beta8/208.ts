...
// Issue #208: Beta.8 arrival evidence for the committed AI inference families
// (research #216 xAI, #217 Replicate, #218 Groq, #220 OpenRouter; routed in #208's
// handoff comment). None has a product detector at the pinned registry revision,
// so each is an arrival family. All four are arrival/provisional evidence: #205's
// safe observation intake does not exist yet, so no contract here is empirical,
// and the hands-on issuance checklists in #216–#220 remain human-only corroboration.
// Owned by that issue only; see docs/specs/beta8-evidence.md.
export const issue = 208;

const researched = '2026-09-24';
const NO_DETECTOR = 'No product detector exists for this family at the pinned registry revision (benchmarks/detectors.json sourceRevision 2b98027); the product counterpart is redact-secret/redact-secret#727, blocked by #726.';

/** Families measured here that no registry detector targets. Their ids are case targets, never detector ids. */
export const arrivalFamilies: ArrivalFamily[] = [
  { id: 'replicate-api-token', taxonomy: 'replicate:api-token', issue, reason: NO_DETECTOR },
  { id: 'groq-api-key', taxonomy: 'groq:api-key', issue, reason: NO_DETECTOR },
  { id: 'xai-api-key', taxonomy: 'xai:api-key', issue, reason: NO_DETECTOR },
  { id: 'openrouter-api-key', taxonomy: 'openrouter:api-key', issue, reason: NO_DETECTOR },
];
...
export const contracts: Record<string, FormatContract> = {
  'replicate-api-token': {
    ...
    review: 'Arrival evidence (#208, research #217). T1 because every frozen field a positive depends on (prefix, 40-character total) is provider-stated. Positives use an alphanumeric body, which every source admits; the body alphabet stays provisional and no fixture asserts silence on a - or _ body byte.',
    fields: [
...
export const profiles: Record<string, FixtureProfile> = {
  ...
  'groq-api-key': 'arrival-24',
  'xai-api-key': 'arrival-24',
