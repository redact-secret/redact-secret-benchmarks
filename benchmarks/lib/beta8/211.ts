...
export const arrivalFamilies: ArrivalFamily[] = [
  { id: 'github-fine-grained-pat', taxonomy: 'github:fine-grained-personal-access-token', issue,
    reason: 'The taxonomy maps no detector to this family: the registry github-token contract is the 2021 ghp_/gho_/ghu_/ghs_/ghr_ + 36 scheme, which github_pat_ does not fit. The product ships github_pat_ matching inside its shared github-token detector (redact-secret#517), but a shared implementation does not make this the same family; findings are scored by span, not by which detector fired. The product documents a github_fine_grained_personal_access_token finding type for github_pat_, still inside the shared github-token detector (redact-secret docs/reference/detection.md at main 10263e5), so the adapter labels those findings with this family (#251).' },
