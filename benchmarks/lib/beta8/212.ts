...
export const arrivalFamilies: ArrivalFamily[] = [
  { id: 'pinecone-api-key-legacy', taxonomy: 'pinecone:legacy-api-key', issue,
    reason: 'No product detector exists for Pinecone at the pinned registry revision 2b98027. The legacy key is a bare UUID, lexically identical to Pinecone\'s own project/key/service-account ids, so it is measured separately and only in context (#228). Still an arrival family at the registry pin f2082ab: the product pinecone-api-key detector (redact-secret#730) claims only the pcsk_ shape and deliberately leaves the legacy bare UUID unclaimed.' },
