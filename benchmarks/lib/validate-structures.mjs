...
export function validateStructures(fixtures) {
  ...
  for (const f of selected.filter(f => f.assessment.contract === 'jwt')) {
    for (const r of f.expected) {
      ...
      if (parts.length !== 3 || JSON.parse(Buffer.from(parts[0], 'base64url')).alg !== 'EdDSA' || !keys.some(key => verify(null, Buffer.from(parts.slice(0, 2).join('.')), key, Buffer.from(parts[2], 'base64url')))) throw new Error(`Invalid reviewed JWT signature: ${f.id}`);
