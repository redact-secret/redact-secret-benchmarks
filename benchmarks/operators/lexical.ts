...
const supports: Operator['supports'] = c => secrets(c.seed).length === 1 &&
  ['T1', 'T2'].includes(c.seed.assessment.tier) &&
  Boolean(contracts[c.seed.assessment.contract ?? '']?.pattern);

function mutate(c: EvaluationCase, change: (value: string) => string): ReturnType<Operator['generate']> {
...
export const lexicalOperators: Operator[] = mutations.map(([id, change]) => ({ id, version: 1, supports, generate: c => mutate(c, change) }));
