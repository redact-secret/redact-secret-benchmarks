...
import type { Candidate, Counts } from '../holdout/types.ts';
...
async function main() {
  ...
  const development = await runEvaluation({ cases, methods, operators, scanners, runId, provenance, onProgress: console.log });
  ...
  const coverage = suite.methods.map((method: string) => {
    ...
    return { method, cases: results.length, variants: results.reduce((n, r) => n + r.variants.length, 0),
      ...
      scanners: development.scanners.map(s => {
        const assertions: Counts = { pass: 0, fail: 0, 'review-required': 0 };
  ...
  const complete = coverage.every((c: any) => c.cases > 0 && c.generationErrors === 0 && c.scanners.every((s: any) => s.status === 'complete')) && holdout.status === 'complete';
  const report = {
    schemaVersion: 1, reportType: 'qualification', engineVersion: '1.0.0', suiteId: 'engine-v1', suiteHash: hash(suite),
