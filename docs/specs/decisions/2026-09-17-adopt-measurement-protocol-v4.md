...
`docs/specs/measurement-v4.md` identifies six defects in the v3 protocol: exact-range
...
- **Baselines** are `baselines/<version>.json`, saved only from a complete run
  (`run.json` with `partial: false`). `docs/generated/release-comparison.md` is generated
  from them; `docs/reports/beta-4/results.md` and `docs/reports/beta-3/results.md` remain as
