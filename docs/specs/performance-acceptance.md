...
(version `3`, hash `ca96dddd…`). The `f2082ab` criteria had themselves been
re-derived after that run's only failure, `suite:accuracy-corpus-identity-mismatch`,
which the maintainer accepted because product
[#762](https://github.com/redact-secret/redact-secret/pull/762) intentionally
re-pinned the corpus's Supabase value; a timing or memory failure would not
have been grounds to re-derive. The recalibration is not re-checked against its own run for a
