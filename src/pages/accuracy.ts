...
export function accuracy(report: Report): string {
  ...
  const breakdown = complete.length
    ? `<section class="panel"><div class="section-heading"><div><h2>Coverage by group</h2><p>Exact matches / expected spans, or clean / negative files. False positives are shown separately.</p></div></div><div class="table-scroll"><table><thead><tr><th>Fixture group</th><th>Files</th>${complete.map((s) => `<th>${e(s.name)}</th>`).join("")}</tr></thead><tbody>${groups
        .map(
          (group) =>
            `<tr><td>${e(group)}</td><td>${complete[0].rows!.filter((r) => r.group === group).length}</td>${complete
              .map((s) => {
                ...
                return `<td><strong>${expected ? `${tp} / ${expected} exact` : `${tn} / ${rows.length} clean`}</strong><small>${fp} false positives</small></td>`;
              })
              .join("")}</tr>`,
        )
  ...
  <section class="panel"><div class="section-heading"><div><h2>Exact-range scores</h2><p>Measures precise span selection, including range disagreements</p></div><span class="tag">LOCAL / OFFLINE VERIFICATION</span></div>
  ...
  <section class="panel"><div class="section-heading"><div><h2>Scanner results</h2><p>Counts are secret spans; true negatives count clean files.</p></div></div><div class="table-scroll"><table><thead><tr><th>Scanner</th><th>Exact TP</th><th>Exact FP</th><th>Exact FN</th><th>Clean negatives</th><th>Contained / expected</th><th>Broader only</th><th>Elapsed*</th></tr></thead><tbody>${report.scanners.map((s) => `<tr><td><strong>${e(s.name)}</strong><small>${e(s.mode)}</small></td>${["tp", "fp", "fn", "tn"].map((k) => `<td>${s.status === "complete" ? s[k as "tp"] : "—"}</td>`).join("")}<td>${s.status === "complete" ? `${s.contained} / ${report.expectedCount}` : "—"}</td><td>${s.status === "complete" ? s.broader : "—"}</td><td>${s.durationMs == null ? "—" : `${s.durationMs.toFixed(1)} ms`}</td></tr>`).join("")}</tbody></table></div><p class="footnote">Containment counts each expected span fully enclosed by any finding once. Broader only excludes exact matches. Neither is a redaction accuracy or precision rate. * One diagnostic wall-clock measurement, including adapter overhead. This is not a throughput benchmark.</p></section>
  ${releaseContext}
  ${breakdown}
            ...
            (row) =>
              `<tr data-search="${e(`${row.id} ${row.group}`.toLowerCase())}"><td><strong><a class="text-link" href="/fixture/${e(report.category)}--${e(row.id)}">${e(row.id)} ↗</a></strong><small>${e(row.group)}</small></td><td>${row.expected.length ? `${row.expected.length} secret span(s)` : "Negative control"}<small>${row.expected.map((r) => `[${r.start}, ${r.end})`).join(" · ")}</small></td>${complete
                .map((s) => {
                  ...
                  return `<td><span class="outcome ${r.fp || r.fn ? "miss" : "pass"}">${r.fp || r.fn ? `${r.fp} exact FP / ${r.fn} exact FN` : "Exact match"}</span><small>Contained: ${r.contained}/${r.expected.length} · broader only: ${r.broader}</small><small>${r.actual.map((a) => `[${a.start}, ${a.end})`).join(" · ") || "No findings"}</small></td>`;
                })
                .join("")}</tr>`,
          )
          .join(
