...
function redactRow(s: Summary) {
  ...
  return `${rowOpen(s)}${scannerCell(s, diag)}<td>${num(g.files)} of ${s.selectedCount}</td><td title="${g.leakedSpans} leaked spans of ${g.spans} · ${g.leakedBytes} of ${g.secretBytes} secret bytes left readable (${percent(g.leakedByteRate, 2)})">${g.leakedSpans ? `${num(g.leakedSpans)} of ${g.spans} <small>${percent(g.leakedSpanRate)}</small>` : `<span class="good">none</span> of ${g.spans}`}</td><td title="${g.collateralBytes} redacted bytes outside every allowed range, against ${g.secretBytes} secret bytes">${g.collateralBytes ? `${num(g.collateralBytes)} B extra <small>${ratio(g.collateralRatio)}× the secrets</small>` : '<span class="good">none</span><small>no extra text destroyed</small>'}</td><td title="pairs where the real secret was covered and its one-character fake stayed clean">${g.twins.pairs ? `${num(g.twins.discriminated)} of ${g.twins.pairs} pairs <small>${percent(g.twins.rate)}</small>` : '<span class="empty">no fakes authored</span>'}${untwinned ? `<small>${plural(untwinned, 'secret')} without a fake yet</small>` : ''}</td><td class="bar-cell">${histogram(g)}</td></tr>`;
}
function controlRow(s: Summary) {
  ...
  return `${rowOpen(s)}${scannerCell(s, `${g.diagnostics.exact.fp} findings on control files · `)}<td>${num(g.files)} of ${s.selectedCount}</td><td>${g.flaggedFiles ? `${num(g.flaggedFiles)} of ${g.files} <small>${percent(g.falseAlarmRate)}</small>` : `<span class="good">none</span> of ${g.files}`}</td><td>${g.meanFindingsPerFlagged == null ? '—' : g.meanFindingsPerFlagged.toFixed(2)}</td><td class="bar-cell">${controlBar(g)}</td></tr>`;
}
