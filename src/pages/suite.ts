...
export function suitePage(data: BenchData, fixtures: Fixture[], id: string): string {
  ...
  const crumb = evidenceCrumb([{ label: 'Coverage', href: '/coverage' }, { label: 'Development history', href: '/coverage#development-history' }, { label: category?.title ?? id }]);
  ...
  const head = `${crumb}<div class="page-head"><div><p class="eyebrow">DEVELOPMENT HISTORY · EXECUTION SUITE</p><h1>${e(category.title)}</h1><div class="meta"><span><b>${selected.length.toLocaleString('en-US')}</b> fixtures</span>${report ? `<span>Run <b>${e(report.runId.slice(0, 10))}</b>${stale ? ` ${statusMark('review', 'Older run')}` : ''}</span><a href="/results/${e(id)}.json" download>Export JSON</a>` : ''}</div></div></div><p class="prose">${e(category.description)}</p><p class="small">This source suite preserves fixture adoption and execution provenance. Test questions are tracked separately as scenarios; suite membership does not redefine them.</p>`;
