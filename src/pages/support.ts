...
function fixtureDebt(matrix: SupportMatrixFile): string {
  ...
  const rows = matrix.families.filter(entry => entry.fixtureProfile && entry.detectors.length && !seen.has(entry.detectors[0]) && seen.add(entry.detectors[0]))
    .map(entry => ({ id: entry.detectors[0], p: entry.fixtureProfile! }))
    .sort((a, b) => b.p.debt.reduce((t, d) => t + d.shortfall, 0) - a.p.debt.reduce((t, d) => t + d.shortfall, 0) || a.id.localeCompare(b.id));
  if (!rows.length) return '';
  const cells = (p: SupportMatrixEntry['fixtureProfile'] & object) => `${n(p.cells.totalFixtures)} · ${n(p.cells.positiveCases)} · ${n(p.cells.benignControls)} · ${n(p.cells.twinPairs)}`;
