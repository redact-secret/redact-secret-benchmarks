...
export function qualificationPage(data: BenchData, evaluation: EvaluationReport | null): string {
  ...
  const head = `${evidenceCrumb([{ label: 'Workbench', href: '/workbench' }, { label: 'Qualification' }])}<div class="page-head"><div><h1>Qualification floors</h1><div class="meta"><span>Floors from <code>qualification/${e(suite.id)}</code> accounting ${e(suite.accounting.version)}</span>${q ? `<span>Evidence run <b>${e(q.finishedAt.slice(0, 10))}</b></span><span>${statusMark(q.status === 'execution-qualified' ? 'pass' : 'review', q.status)}</span>` : ''}</div></div></div>`;
  const evidence = q
