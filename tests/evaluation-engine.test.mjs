...
import { createRegistry } from '../benchmarks/engine/registry.mjs';
import { createMethods } from '../benchmarks/methods/index.mjs';
import { createOperators } from '../benchmarks/operators/index.mjs';
import { loadCases } from '../benchmarks/engine/cases.mjs';
import { generateCase, hash, bytes, secrets, validateCase } from '../benchmarks/engine/model.mjs';
import { mapFixture } from '../benchmarks/operators/context.mjs';
import { absolute, observe, relation } from '../benchmarks/engine/assertions.mjs';
import { runEvaluation, exitCode } from '../benchmarks/engine/runner.mjs';
import { generate, evaluate } from '../benchmarks/methods/common.mjs';
...
test('all existing corpora bridge deterministically into five methods and all detector targets', () => {
  ...
  assert.equal(new Set(cases.flatMap(c => c.targets)).size, 25);
  assert.equal(cases.filter(c => c.method === 'twin').length, 56);
