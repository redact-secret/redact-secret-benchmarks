...
test('benchmarks/ledger-decisions.json claims every operator class this record reclassifies', async () => {
...
test('#125: benchmarks/ledger-decisions.json claims the differential pending-fixture decision class', async () => {
  const decided = await decidedOperators();
...
test('a not-assertable entry under a decision= class passes only when the data file claims that class', () => {
  ...
  assert.deepEqual(problems, ['undecided: marked not-assertable for class "decision=differential.made-up", which benchmarks/ledger-decisions.json records no decision for']);
});
...
test('a not-assertable entry for an undecided operator class fails the gate', () => {
  ...
  assert.deepEqual(problems, ['undecided: marked not-assertable for class "operator=lexical.length-plus-one", which benchmarks/ledger-decisions.json records no decision for']);
});
...
test('a not-assertable entry carrying no operator class at all fails the gate, not just an undecided one', () => {
  ...
  assert.deepEqual(undecidedNotAssertableEntries(ledger, new Map()), ['a: marked not-assertable for class "t0-pending-fixture", which benchmarks/ledger-decisions.json records no decision for']);
});
