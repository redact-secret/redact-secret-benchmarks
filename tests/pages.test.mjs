...
test('Coverage: detectors by fixture count with the minimum sample size drawn on every bar', async () => {
  ...
  assert.equal(probe.counts.discriminated + probe.counts['not-discriminated'], 43, 'the test product reports every secret exactly and nothing else');
  ...
  assert.ok(text(coveragePage(fixtures, 'all')).includes('43 not measured'), 'without a run nothing is claimed');
